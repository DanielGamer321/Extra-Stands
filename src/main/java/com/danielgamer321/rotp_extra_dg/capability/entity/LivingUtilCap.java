...
public class LivingUtilCap {
    ...
    public boolean userString(LivingEntity user) {
        if (StoneFreeUserStringAttack.getLandedString(user) || StoneFreeUserStringSweep.getLandedString(user) ||
                StoneFreeUserStringWhip.getLandedString(user) || StoneFreeUserStringBind.getLandedString(user) ||
                StoneFreeUserStringPick.getLandedString(user) || StoneFreeUserGrapple.getLandedString(user)) {
            userString = false;
        }
        else {
            userString = true;
    ...
    public void setPlacedBarriersCount(int barriers) {
        ...
        if (this.PLACED_BARRIERS != barriers) {
            ...
            if (!entity.level.isClientSide()) {
                PacketManager.sendToClientsTrackingAndSelf(new TrPlacedBarriersCountPacket(entity.getId(), barriers), entity);
            }
        }
    }
    ...
    public void setBarriersRemovedCount(int barriers) {
        ...
        if (this.BARRIERS_REMOVED != barriers) {
            ...
            if (!entity.level.isClientSide()) {
                PacketManager.sendToClientsTrackingAndSelf(new TrBarriersRemovedCountPacket(entity.getId(), barriers), entity);
            }
        }
    }
    ...
    public void onTracking(ServerPlayerEntity tracking) {
        PacketManager.sendToClient(new TrPlacedBarriersCountPacket(entity.getId(), PLACED_BARRIERS), tracking);
        PacketManager.sendToClient(new TrBarriersRemovedCountPacket(entity.getId(), BARRIERS_REMOVED), tracking);
    }

    public void setMobiusStripMode(boolean Status) {
        this.mobiusStripMode = Status;
    }

    public boolean getMobiusStripMode() {
        return mobiusStripMode;
    }

    public CompoundNBT toNBT() {
        CompoundNBT nbt = new CompoundNBT();
        nbt.putBoolean("BlockingItemsStatus", blockingItemsStatus);
        nbt.putBoolean("MobiusStripMode", mobiusStripMode);
        return nbt;
    }

    public void fromNBT(CompoundNBT nbt) {
        this.blockingItemsStatus = nbt.getBoolean("BlockingItemsStatus");
        this.mobiusStripMode = nbt.getBoolean("MobiusStripMode");
    }
}
